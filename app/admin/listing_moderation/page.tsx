...
export default async function AdminListingModerationPage() {
  const items = await getAdminQueueItems("listing_moderation");
  ...
  return (
    <main className="mx-auto w-full max-w-3xl flex-1 p-6">
      ...
      <div className="mb-6">
        <h1 className="display text-2xl text-fg">Listing moderation</h1>
      ...
      <div className="space-y-3">
        {items.map((item) => {
          ...
          const reason = (meta?.reason as string) ?? "Not specified";
          return (
            <div key={item.id} className="card p-4">
              <div className="flex items-start justify-between gap-4">
                <div className="min-w-0">
                  <p className="font-medium text-fg">Listing moderation request</p>
        ...
        {items.length === 0 && (
          <div className="card p-12 text-center">
            <p className="text-muted">No pending listing moderation requests.</p>
          </div>
        )}
      </div>
    </main>
